...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghostDists = util.Counter()
        for beliefs in livingGhostPositionDistributions:
            pos = beliefs.argMax()
            ghostDists[pos] = 1/self.distancer.getDistance(pacmanPosition, pos)

        afterPos = ghostDists.argMax()
        minDist = 9999
        bestAct = None
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            nextDist = self.distancer.getDistance(nextPos, afterPos)
            if nextDist < minDist:
                minDist = nextDist
                bestAct = action
